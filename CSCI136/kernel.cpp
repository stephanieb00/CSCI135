...
Assignment:Task E. Scale 200%.,Lab8E

This program will read a PGM image from the file "inImage.pgm", and outputs a modified image to "outImage.pgm".cale the original picture to 200% of its size. It can be done by increasing the size of the picture by the factor of 2, and copying each pixel of the input as a small 2x2 square in the output.
*/
...
/*void scale(int image[MAX_H][MAX_W], int height, int width){
	//int scale; // want to find the average;
	//height = height * 2;
	//width = width * 2;
	for (int row = 0; row < height; row++) {
		...
		for (int col = 0; col < width; col++) {
			...
			image[row*2+1][col*2] = image[row][col];
			image[row*2+1][col*2+1] = image[row][col];
			image[row*2][col*2+1] = image[row][col]	;
